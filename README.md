...
このツールは予約一覧の赤い `詳細` ボタンを上から順番に開き、詳細画面の中で `領収書` ボタンを探します。領収書がない予約はスキップします。JR九州サイトはブラウザの戻る操作を嫌うため、各詳細の確認後は一覧URLを再読み込みして次の予約へ進みます。
...
画面上のボタン名が変わった場合は、`receiptLinkPatterns` または `detailButtonPatterns` に新しい文言を追加してください。